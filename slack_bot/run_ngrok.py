...
app = App(
    token=env_settings.app_config_token.get_secret_value()
)

def log_event_callback(log):
    logging.debug("Ngrok log: %s", log)
...
def setup_ngrok():
    ...
    try:
        ...
        logging.info("Running ngrok tunnel %s -> http://localhost:%s", public_url, port)
        update_bot_manifest(public_url)
    except PyngrokNgrokError as e:
        logging.error("Ngrok exception: ", exec=e)
